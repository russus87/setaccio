...
const LOTTO: &str = "/home/russus/Scaricati/T1Q73KFP";
...
fn il_lotto_reale_viene_indicizzato_e_classificato() {
    let Some(db) = scandisci(LOTTO, Fascia::Tracciati) else {
        return;
    };
    ...
    // cui Setaccio esiste.
    let tracciato = db
        .file_per_path(&format!("{LOTTO}/328102"))
        .expect("query")
    ...
    let pdf = db
        .file_per_path(&format!("{LOTTO}/T1Q73KFP_0000001.pdf"))
        .expect("query")
...
fn il_tracciato_si_riconosce_anche_in_una_sorgente_documenti() {
    let Some(db) = scandisci(LOTTO, Fascia::Documenti) else {
        return;
    };
    let t = db
        .file_per_path(&format!("{LOTTO}/328102"))
...
fn la_ricerca_trova_il_testo_dentro_il_pdf_del_lotto() {
    ...
    assert_eq!(per_nome.len(), 1, "il tracciato si deve trovare per nome");
}
